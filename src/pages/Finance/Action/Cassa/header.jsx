...
const Header = ({ is_filter_open, set_is_filter_open, set_is_add_modal_open }) => {
    ...
    const [human_value, set_human_value] = useState("Tranzaksiya");
    const [student_value, set_student_value] = useState("To'lov turi");
    const [exercise_value, set_exercise_value] = useState("O'qituvchi");
    ...
    return (
        <div className="w-full">
            <div className={`flex flex-row justify-end gap-[10px] pl-[130px] transition-all duration-[0.3s] ease-out translate-y-0 h-auto pt-[25px] -mb-[5px] opacity-100`}>
                <button className="flex justify-center items-center gap-[10px] w-[162px] h-[66px] bg-[#0D99FF] rounded-[5px]">
                    <img src={up_icon} />
                    <h1 className="font-roboto font-[400] text-[20px] leading-[23.44px] text-white">Export</h1>
                </button>

                <div
                    className="w-[244px] h-[67px] bg-white rounded-[5px] relative"
                >
                    <div
                    ...
                    <div
                        className={`absolute top-[60px] left-0 w-full bg-white border-[#C5C5C5] shadow-md rounded-[5px] z-10 overflow-hidden transition-all duration-[300ms] ease-out ${is_calendar_open ? `h-[286px] mt-[5px] border-[1.5px] overflow-hidden menu_scroll` : 'h-0 border-[0px]'
                            }`}
                    >
                ...
                <div className="w-[244px] h-[67px] bg-white rounded-[5px] relative">
                    <div className={`h-[67px] w-[244px] cursor-pointer flex items-center border-[#C5C5C5] ${is_human_open ? 'border-[1.5px]' : 'border-[0px]'} rounded-[5px] justify-between ${human_value.length > 15 ? 'px-[20px]' : 'px-[28px]'}`} onClick={() => set_is_human_open(!is_human_open)}>
                    ...
                    <div
                        className={`absolute top-[60px] left-0 w-full bg-white border-[#C5C5C5] shadow-md rounded-[5px] z-10 overflow-hidden transition-all duration-[300ms] ease-out ${is_human_open ? `h-[250px] mt-[5px] border-[1.5px] ${humans_list.length > 4 ? 'overflow-y-scroll' : 'overflow-y-hidden'} menu_scroll` : 'h-0 border-0'
                            }`}
                    >
                ...
                >
                    <div
                        className={`h-[67px] w-[244px] cursor-pointer flex items-center border-[#C5C5C5] ${is_student_open ? 'border-[1.5px]' : 'border-[0px]'} rounded-[5px] justify-between ${student_value.length > 15 ? 'px-[20px]' : 'px-[28px]'}`}
                ...
                <div
                    className="h-[67px] w-[244px] bg-white rounded-[5px] relative"
                >
                    <div
                        className={`h-[67px] w-[244px] cursor-pointer flex items-center border-[#C5C5C5] ${is_exercise_open ? 'border-[1.5px]' : 'border-[0px]'} rounded-[5px] justify-between ${exercise_value.length > 15 ? 'px-[20px]' : 'px-[28px]'}`}
                    ...
                    <div
                        className={`absolute top-[60px] left-0 w-full bg-white border-[#C5C5C5] shadow-md rounded-[5px] z-10 overflow-hidden transition-all duration-[300ms] ease-out ${is_exercise_open ? `h-[250px] mt-[5px] border-[1.5px] ${exercises_list.length > 4 ? 'overflow-y-scroll' : 'overflow-y-hidden'} menu_scroll` : 'h-0 border-0'
                            }`}
                    >
                        <div className="flex justify-center mb-2 pt-[15px]">
                            <input
